...
if st.button("Calculate Remaining Monthly Income and Budget Allocation"):
    ...
    st.markdown(f"- Needs ({need_percentage * 100}%): ${theo_needs:.2f}")
    st.markdown(f"- Wants ({wants_percentage * 100}%): ${theo_wants:.2f}")
    st.markdown(f"- Savings/Debt Repayment ({savings_percentage * 100}%): ${theo_savings:.2f}")
